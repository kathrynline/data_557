...
sixmonth_data = na.omit(read.csv(".\\prepped_data\\six_month_outlier_screened.csv", check.names = FALSE))
...
```{r}
round(summary(lm(cfratio ~ overall + factor(gdp_pc_quartile), data = sixmonth_data))$coefficients,6)
par(mfrow=c(2,2),mar=c(5,4,2,1))
...
```{r}
round(summary(lm(casepc ~ overall + gdp_pc, data = sixmonth_data))$coefficients,6)
par(mfrow=c(2,2),mar=c(5,4,2,1))
...
```{r}
round(summary(lm(deathpc ~ overall + gdp_pc, data = sixmonth_data))$coefficients,6)
par(mfrow=c(2,2),mar=c(5,4,2,1))
...
```{r}
round(summary(lm(cfratio ~ overall + gdp_pc, data = sixmonth_data))$coefficients,6)
par(mfrow=c(2,2),mar=c(5,4,2,1))
...
\newpage
``` {r}
summary(lm(cfratio ~ gdp_pc, data = sixmonth_data))
```
...
\newpage
``` {r}
summary(lm(cfratio ~ gdp_pc_quartile, data = sixmonth_data))
```